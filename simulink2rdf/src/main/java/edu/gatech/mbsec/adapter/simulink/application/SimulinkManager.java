	...
	public static synchronized void loadSimulinkWorkingDirectory() {
		...
		Thread thread = new Thread() {
			public void start() {
				...
				try{
					// get file path of folder containing simulink file
					Path pathToCurrentDirectory = Paths.get("").toAbsolutePath();
					// alternative way: String workingDir = System.getProperty("user.dir");
					
					
					// load Simulink XMI file
					Resource ecoreResource = loadEcoreModel(org.eclipse.emf.common.util.URI.createFileURI(
//							new File(OSLC4JSimulinkApplication.simulinkModelPath + "/simulinkWorkDir.xmi")
//									.getAbsolutePath()));
					new File(pathToCurrentDirectory + "\\simulinkWorkDir.xmi")
					.getAbsolutePath()));
...
//									System.out.println(targetPort.getId());
		...
		try {
			...
			// delete temp Simulink XMI file			
			Path pathToCurrentDirectory = Paths.get("").toAbsolutePath();
			new File(pathToCurrentDirectory + "\\simulinkWorkDir.xmi").delete();
...
//		public static List<SimulinkParameter> oslcSimulinkParameters = new ArrayList<SimulinkParameter>();
		...
		objectList.addAll(oslcSimulinkModels);
		objectList.addAll(oslcSimulinkBlocks);
		objectList.addAll(oslcSimulinkInputPorts);
		objectList.addAll(oslcSimulinkOutputPorts);
		objectList.addAll(oslcSimulinkParameters);
		objectList.addAll(qNameOslcSimulinkLineMap.values());
		
		int arraySize = oslcSimulinkModels.size() + oslcSimulinkBlocks.size() + oslcSimulinkInputPorts.size() + oslcSimulinkOutputPorts.size() + oslcSimulinkParameters.size() + qNameOslcSimulinkLineMap.values().size();
		Object[] objects = new Object[arraySize];		
		objects = objectList.toArray();
		
		if(Simulink2RDF.outputMode.equals("rdfxml")){
			// print RDF in file
			try {
		        ...
		        File newFile = new File(Simulink2RDF.rdfFileLocation);
		        newFile.createNewFile();
		        
		        OutputStream outputStream = new FileOutputStream(Simulink2RDF.rdfFileLocation);	      	       	        
		        writer.write(model,
		                     outputStream,
		                     null);
		...
		else if(Simulink2RDF.outputMode.equals("jenatdb")){
			try {
				if(!new File(Simulink2RDF.tdbdir).exists()){
					Files.createDirectories(Paths.get(Simulink2RDF.tdbdir));
				}
				
				
				com.hp.hpl.jena.rdf.model.Model model = JenaModelHelper.createJenaModel(objects);
				com.hp.hpl.jena.query.Dataset dataset = com.hp.hpl.jena.tdb.TDBFactory.createDataset(Simulink2RDF.tdbdir);
