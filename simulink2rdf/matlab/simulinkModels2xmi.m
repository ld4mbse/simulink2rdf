...
function simulinkModels2xmi(simulinkModelPaths)

disp('command line version for Simulink models')
disp(simulinkModelPaths)


try
% open folder with Simulink models
%cd(simulinkModelPath);

% helper functions can be accessed via the SimulinkXMIUtil struct
util = SimulinkXMIUtil;

% load simulink model in workspace (old way)
...
celldata = cellstr(simulinkModelPaths);
for i = 1 : length(celldata) 

%for i = 1 : length(simulinkModelPaths)     
    
    simulinkModelPath = celldata(i);
    %simulinkModelPath = simulinkModelPaths(i);

    % get directory name
    %file = strtrim(cell{1,i});
    file = simulinkModelPath{:};
    fileParts = strsplit(file,'\');
    dirName = fileParts{1,1};
    
    % load Simulink model
    %load_system(strtrim(cell{1,i})); 
    load_system(simulinkModelPath);
